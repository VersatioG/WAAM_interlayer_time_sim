...
def update_temperatures_matrix(node_matrix, model, dt, is_welding=False, arc_power=0.0, welding_node_idx=None):
    ...
    # Pre-convert active indices to array for vectorization (OPTIMIZED: avoid list conversion if already cached)
    ...
    # OPTIMIZATION: Precompute radiation constants
    CONST_EPSILON_TABLE_STEFAN = EPSILON_TABLE * STEFAN_BOLTZMANN
    CONST_EPSILON_BP_STEFAN = EPSILON_BP * STEFAN_BOLTZMANN
    CONST_STEFAN = STEFAN_BOLTZMANN
    ...
    # --- Radiation ---
    # OPTIMIZATION: Cache T_amb_K4 (constant), compute T_K4 once
    T_K4 = (T + 273.15)**4
    T_amb_K4 = (AMBIENT_TEMP + 273.15)**4
    # OPTIMIZATION: Cache top_layer_idx - used multiple times
    ...
    if node_matrix.table_grid is not None:
        for table_node_idx in node_matrix.table_indices:
            ...
            if table_rad_area > 0:
                Q_balance[table_node_idx] -= (CONST_EPSILON_TABLE_STEFAN * 
                                              table_rad_area * 
        ...
        Q_balance[node_matrix.table_idx] -= (CONST_EPSILON_TABLE_STEFAN * 
                                              table_rad_area * 
    ...
    if node_matrix.bp_idx is not None:
        ...
        Q_balance[node_matrix.bp_idx] -= (CONST_EPSILON_BP_STEFAN * 
                                          bp_rad_area * 
    ...
    if len(active_indices_arr) > 0:
        ...
        # Calculate radiation (OPTIMIZATION: use precomputed constant)
        Q_rad = epsilon_active * CONST_STEFAN * rad_areas_active * (T_K4[active_indices_arr] - T_amb_K4)
        Q_balance[active_indices_arr] -= Q_rad
    
    # --- Conduction ---
    # OPTIMIZATION: Precompute conduction constants
    CONST_LAMBDA_TABLE = LAMBDA_TABLE
    CONST_LAMBDA_WAAM = LAMBDA_WAAM
    CONST_ALPHA_CONTACT_AREA = ALPHA_CONTACT * CONTACT_BP_TABLE
    
    # Table internal conduction
    if node_matrix.table_grid is not None:
        dx, dy, dz = node_matrix.table_node_dims
        # OPTIMIZATION: Precompute areas and distances for table conduction
        area_x, dist_x = dy * dz, dx
        area_y, dist_y = dx * dz, dy
        area_z, dist_z = dx * dy, dz
        const_cond_x = CONST_LAMBDA_TABLE * area_x / dist_x
        const_cond_y = CONST_LAMBDA_TABLE * area_y / dist_y
        const_cond_z = CONST_LAMBDA_TABLE * area_z / dist_z
        ...
        for table_node_idx in node_matrix.table_indices:
            ...
            for direction, neighbor_idx in neighbors.items():
                ...
                if direction in ('x+', 'x-'):
                    q_cond = const_cond_x * (T[neighbor_idx] - T[table_node_idx])
                elif direction in ('y+', 'y-'):
                    q_cond = const_cond_y * (T[neighbor_idx] - T[table_node_idx])
                else:
                    q_cond = const_cond_z * (T[neighbor_idx] - T[table_node_idx])
    ...
    if node_matrix.table_bp_contact_idx is not None and node_matrix.bp_idx is not None:
        ...
        q_contact = CONST_ALPHA_CONTACT_AREA * (T[node_matrix.bp_idx] - T[node_matrix.table_bp_contact_idx])
    ...
    if node_matrix.bp_idx is not None:
        ...
        if first_layer_nodes:
            # OPTIMIZATION: Precompute constants for BP-layer conduction
            dist_bp_layer = (BP_THICKNESS / 2) + (LAYER_HEIGHT / 2)
            lam_bp_layer = (LAMBDA_BP + LAMBDA_WAAM) / 2
            const_bp_layer = lam_bp_layer / dist_bp_layer
            for target_idx in first_layer_nodes:
                ...
                q_vert = const_bp_layer * area * (T[target_idx] - T[bp_idx])
    ...
    const_horiz_base = CONST_LAMBDA_WAAM * LAYER_HEIGHT / const_horiz_dist
    const_long_dist = TRACK_LENGTH / node_matrix.num_elements_per_bead
    ...
    for i in active_waam_indices_list:
        ...
        if h_right is not None:
            # OPTIMIZATION: Precompute based on element type
        ...
        if l_forward is not None:
            # OPTIMIZATION: Use precomputed element_length
